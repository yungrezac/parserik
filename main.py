...
def stream_parser(seller_id, brand_id, category):
    ...
    try:
        ...
        if not products_total:
            yield json.dumps({'type': 'error', 'message': 'Товары не найдены. Проверьте правильность ID продавца и бренда.'})
            return
        ...
        while current_page <= pages_count:
            ...
            try:
                ...
                yield json.dumps({'type': 'log', 'message': f'Ошибка на странице {current_page}: {str(e)}. Пропускаем...'})
                current_page += 1
                time.sleep(random.uniform(2, 4))
                continue

            if not products_on_page:
                yield json.dumps({'type': 'log', 'message': f'Страница {current_page} пуста. Завершаем парсинг.'})
                break
            ...
            for item in products_on_page:
                ...
                yield json.dumps({
                    ...
                    'message': item.get('name', '')[:50] + '...' if len(item.get('name', '')) > 50 else item.get('name', '')
                })
                ...
                while True:
                    ...
                    try:
                        productResponse = session.get(urlItem, timeout=5)
                        if productResponse.status_code == 200:
                            item_advanced_data = productResponse.json()
                            break
                ...
                item['advanced'] = item_advanced_data
                all_products.append(item)
                
                # Более мягкая задержка между товарами
                if count % 10 == 0:  # Задержка каждые 10 товаров
                    time.sleep(random.uniform(0.2, 0.5))

            current_page += 1
            # Увеличиваем задержку между страницами
...
def get_mediabasket_route_map():
    try:
        response = make_request('https://cdn.wbbasket.ru/api/v3/upstreams', headers=headers, timeout=10)
        data = response.json()
        ...
        print(f"Ошибка получения карты маршрутов: {e}")
        return []
    return []
...
def get_host_by_range(range_value, route_map):
    ...
    for host_info in route_map: 
        if ('vol_range_from' in host_info and 'vol_range_to' in host_info and 
            host_info['vol_range_from'] <= range_value <= host_info['vol_range_to']): 
...
def find_options_by_group_name(grouped_options, group_name): 
    try: 
        return next((g['options'] for g in grouped_options if g['group_name'] == group_name), [])
